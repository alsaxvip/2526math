...
const StudentDashboard = () => {
  ...
  const getCharacterClass = (level: number, averageGrade: number) => {
    ...
    if (level >= 15 && averageGrade >= 93) return { name: 'Math Paladin', icon: '💂🏻‍♂️', color: 'from-blue-600 to-purple-700' };
    if (level >= 10 && averageGrade >= 90) return { name: 'Math Knight', icon: '⚔️', color: 'from-green-600 to-blue-600' };
    if (level >= 6 && averageGrade >= 88) return { name: 'Math Apprentice', icon: '📖', color: 'from-yellow-600 to-green-600' };
    if (level >= 3 && averageGrade >= 85) return { name: 'Math Initiate', icon: '📚', color: 'from-orange-600 to-yellow-600' };
